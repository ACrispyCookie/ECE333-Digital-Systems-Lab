module vga_controller (
    ...
    .write_enable(1'b0), .write_address(14'd0), .write_data(3'd0));
    ...
    vsync_controller_inst(.clk(new_clk), .reset(debounced_reset), .enable(debounced_enable), .sync(vga_vsync), .rgb_enabled(vrgb_enabled), .pixel(vpixel), .upscale_counter(), .frame_end());
