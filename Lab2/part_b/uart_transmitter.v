module uart_transmitter (clk, reset, tx_data, baud_select, tx_en, tx_wr, txD, tx_busy);
    ...
    wire baud_sample;
    wire is_transmitting = current_state != DISABLED && current_state != IDLE;
    wire tx_sample = sample_counter == 4'b1111 && baud_sample == 1'b1;
    ...
    baud_controller baud_controller_tx(.clk(clk), .reset(reset), .enable(is_transmitting), .baud_select(baud_select), .enable_sample(baud_sample));
    ...
    always @(tx_en or tx_wr or tx_sample or current_state) begin
        case (current_state)
            DISABLED: begin
            ...
            BIT_7: begin
                next_state = tx_sample ? PARITY_BIT : BIT_7;
            end
            PARITY_BIT: begin
