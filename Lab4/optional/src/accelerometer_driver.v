...
enable, edit_mode, accel_mode, up_ctrl, down_ctrl, left_ctrl, right_ctrl,
vga_red, vga_green, vga_blue, vga_hsync, vga_vsync);

input clk, reset, miso;
input enable, edit_mode, accel_mode;
...
vga_controller vga_controller_inst(.clk(clk), .reset(reset), .enable(enable), .edit_mode(edit_mode), .accel_mode(accel_mode), 
.x_accel(X_val), .y_accel(Y_val), .up_ctrl(up_ctrl), .down_ctrl(down_ctrl), .left_ctrl(left_ctrl), .right_ctrl(right_ctrl),
